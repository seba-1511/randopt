...
def cli(fn=None):
    ...
    if not len(arg_names) == len(arg_defaults):
        print('randopt.cli: some function arguments do not have default values.')
        sys.exit(-1)
    fn_name = fn.__name__
    global __FUNCTIONS
    ...
    global __ARGUMENTS
    __ARGUMENTS[fn_name] = {name: default for name, default in zip(arg_names, arg_defaults)}


def experiment(name='main', directory='randopt_results', fn=None):
    pass
...
def parse():
    ...
    parser = ArgumentParser('Randopt\'s custom argument parser')
    for arg in func_def:
